...
;$.smVersion = "0.6.2";+function ($) {
    ...
    //鍏ㄥ眬閰嶇疆
    var defaults = {
        autoInit: false, //鑷姩鍒濆鍖栭〉闈�
        showPageLoadingIndicator: true, //push.js鍔犺浇椤甸潰鐨勬椂鍊欐樉绀轰竴涓姞杞芥彁绀�
        router: true, //榛樿浣跨敤router
        swipePanel: "left", //婊戝姩鎵撳紑渚ф爮
        swipePanelOnlyClose: true  //鍙厑璁告粦鍔ㄥ叧闂紝涓嶅厑璁告粦鍔ㄦ墦寮€渚ф爮
    };
...
(function($) {
    ...
    //閲嶇疆zepto鐨剆how鏂规硶锛岄槻姝㈡湁浜涗汉寮曠敤鐨勭増鏈腑 show 鏂规硶鎿嶄綔 opacity 灞炴€у奖鍝嶅姩鐢绘墽琛�
    $.fn.show = function(){
        var elementDisplay = {};
...
;(function () {
    ...
    FastClick.prototype.needsClick = function(target) {
        ...
        // 淇bug: 濡傛灉鐖跺厓绱犱腑鏈� label
        // 濡傛灉label涓婃湁needsclick杩欎釜绫伙紝鍒欑敤鍘熺敓鐨勭偣鍑伙紝鍚﹀垯锛岀敤妯℃嫙鐐瑰嚮
        var parent = target;
    ...
    FastClick.prototype.onTouchEnd = function(event) {
        ...
        //淇瀹夊崜寰俊涓嬶紝input type="date" 鐨刡ug锛岀粡娴嬭瘯date,time,month宸叉病闂
...
+function ($) {
    ...
    $.openModal = function (modal, cb) {
        ...
        if (isToast) {
            modal.css({
                marginLeft: - Math.round(modal.outerWidth() / 2 / 1.185) + 'px' //1.185 鏄垵濮嬪寲鏃跺€欑殑鏀惧ぇ鏁堟灉
            });
        }
...
+function ($) {
    ...
    var Calendar = function (params) {
        ...
        function openOnInput(e) {
            e.preventDefault();
            // 瀹夊崜寰俊webviewreadonly鐨刬nput渚濈劧寮瑰嚭杞敭鐩橀棶棰樹慨澶�
    ...
    IScroll.prototype = {
        ...
        resetPosition: function(time) {
            ...
            if (this.options.ptr && this.y > 44 && this.startY * -1 < $(window).height() && !this.ptrLock) {// jshint ignore:line
                ...
                this._execEvent('ptr');
                // 闃叉杩斿洖鐨勮繃绋嬩腑鍐嶆瑙﹀彂浜� ptr 锛屽鑷磋瀹氫綅鍒� 44px锛堝洜涓哄彲鑳絛one浜嬩欢瑙﹀彂寰堝揩锛屽湪杩斿洖鍒�44px浠ュ墠灏辫Е鍙慸one
                this.ptrLock = true;
                var self = this;
                setTimeout(function() {
                    self.ptrLock = false;
 ...
 *  - beforePageSwitch: page åˆ‡æ¢å‰ï¼Œåœ¨ pageAnimationStart å‰ï¼ŒbeforePageSwitch ä¹‹åŽä¼šåšä¸€äº›é¢å¤–çš„å¤„ç†æ‰è§¦å‘ pageAnimationStart
 ...
 *
 * é»˜è®¤åœ°è¿‡æ»¤äº†éƒ¨åˆ†åè®®çš„é“¾æŽ¥ï¼ŒåŒ…æ‹¬ tel:, javascript:, mailto:ï¼Œè¿™äº›é“¾æŽ¥å°†ä¸ä¼šä½¿ç”¨è·¯ç”±åŠŸèƒ½ã€‚å¦‚æžœæœ‰æ›´å¤šçš„è‡ªå®šä¹‰æŽ§åˆ¶éœ€æ±‚ï¼Œå¯ä»¥åœ¨ $.config.routerFilter å®žçŽ°
 *
...
+function($) {
    ...
    var EVENTS = {
        pageLoadStart: 'pageLoadStart', // ajax å¼€å§‹åŠ è½½æ–°é¡µé¢å‰
        pageLoadCancel: 'pageLoadCancel', // å–æ¶ˆå‰ä¸€ä¸ª ajax åŠ è½½åŠ¨ä½œåŽ
        pageLoadError: 'pageLoadError', // ajax åŠ è½½é¡µé¢å¤±è´¥åŽ
        pageLoadComplete: 'pageLoadComplete', // ajax åŠ è½½é¡µé¢å®ŒæˆåŽï¼ˆä¸è®ºæˆåŠŸä¸Žå¦ï¼‰
        pageAnimationStart: 'pageAnimationStart', // åŠ¨ç”»åˆ‡æ¢ page å‰
        pageAnimationEnd: 'pageAnimationEnd', // åŠ¨ç”»åˆ‡æ¢ page ç»“æŸåŽ
        beforePageRemove: 'beforePageRemove', // ç§»é™¤æ—§ document å‰ï¼ˆé€‚ç”¨äºŽéžå†…è” page åˆ‡æ¢ï¼‰
        pageRemoved: 'pageRemoved', // ç§»é™¤æ—§ document åŽï¼ˆé€‚ç”¨äºŽéžå†…è” page åˆ‡æ¢ï¼‰
        beforePageSwitch: 'beforePageSwitch', // page åˆ‡æ¢å‰ï¼Œåœ¨ pageAnimationStart å‰ï¼ŒbeforePageSwitch ä¹‹åŽä¼šåšä¸€äº›é¢å¤–çš„å¤„ç†æ‰è§¦å‘ pageAnimationStart
        pageInit: 'pageInitInternal' // ç›®å‰æ˜¯å®šä¹‰ä¸ºä¸€ä¸ª page åŠ è½½å®Œæ¯•åŽï¼ˆå®žé™…å’Œ pageAnimationEnd ç­‰åŒï¼‰
    };
     ...
     *
     * @param {String} url url
     * @param {Boolean=} ignoreCache æ˜¯å¦å¼ºåˆ¶è¯·æ±‚ä¸ä½¿ç”¨ç¼“å­˜ï¼Œå¯¹ document ç”Ÿæ•ˆï¼Œé»˜è®¤æ˜¯ false
     */
     ...
     * @param {Boolean=} ignoreCache æ˜¯å¦ä¸ä½¿ç”¨ç¼“å­˜å¼ºåˆ¶åŠ è½½é¡µé¢
    ...
    function isInRouterBlackList($link) {
        ...
        //å¦‚æžœéžnoschemeå½¢å¼çš„é“¾æŽ¥ï¼Œä¸”åè®®ä¸æ˜¯http(s)ï¼Œé‚£ä¹ˆè·¯ç”±ä¸ä¼šå¤„ç†è¿™ç±»é“¾æŽ¥
